...
func (s *skelpipePasswordWrapper) TestPassword(conn libplugin.ConnMetadata, password []byte) (bool, error) {
	...
	for _, data := range pwds {
		slog.Debug(fmt.Sprintf("try to match password using htpasswd"))
...
func (s *skelpipePublicKeyWrapper) AuthorizedKeys(conn libplugin.ConnMetadata) ([]byte, error) {
	...
	if s.from.AuthorizedKeysSecret.Name != "" {
		slog.Debug(fmt.Sprintf("mapping to %v authorized keys using secret %v", s.pipe.Spec.To.Host, s.from.AuthorizedKeysSecret.Name))
		...
		for _, k := range []string{anno["sshpiper.com/authorizedkeys_field_name"], anno["authorizedkeys_field_name"], "authorized_keys", "authorizedkeys", "ssh-authorizedkeys"} {
			data := secret.Data[k]
			if data != nil {
				slog.Debug(fmt.Sprintf("found authorized keys in secret %v/%v", s.from.AuthorizedKeysSecret.Name, k))
...
func (s *skelpipeToPrivateKeyWrapper) PrivateKey(conn libplugin.ConnMetadata) ([]byte, []byte, error) {
	slog.Debug(fmt.Sprintf("mapping to %v private key using secret %v", s.to.Host, s.to.PrivateKeySecret.Name))
	...
	for _, k := range []string{anno["sshpiper.com/privatekey_field_name"], anno["privatekey_field_name"], "ssh-privatekey", "privatekey"} {
		data := secret.Data[k]
		if data != nil {
			slog.Debug(fmt.Sprintf("found private key in secret %v/%v", s.to.PrivateKeySecret.Name, k))
			privateKey = data
			break
		}
	}
	...
	if anno["no_ca_publickey"] != "true" && anno["sshpiper.com/no_ca_publickey"] != "true" {
		for _, k := range []string{anno["sshpiper.com/publickey_field_name"], anno["publickey_field_name"], "ssh-publickey-cert", "publickey-cert", "ssh-publickey", "publickey"} {
			data := secret.Data[k]
			if data != nil {
				slog.Debug(fmt.Sprintf("found publickey key cert in secret %v/%v", s.to.PrivateKeySecret.Name, k))
				publicKey = data
				break
			}
		}
...
func (s *skelpipeToPasswordWrapper) OverridePassword(conn libplugin.ConnMetadata) ([]byte, error) {
	if s.to.PasswordSecret.Name != "" {
		slog.Debug(fmt.Sprintf("mapping to %v password using secret %v", s.to.Host, s.to.PasswordSecret.Name))
		...
		for _, k := range []string{anno["password_field_name"], "password"} {
			data := secret.Data[k]
			if data != nil {
				slog.Debug(fmt.Sprintf("found password in secret %v/%v", s.to.PasswordSecret.Name, k))
				return data, nil
			}
		}

		slog.Warn(fmt.Sprintf("password field not found in secret %v", s.to.PasswordSecret.Name))
	}
